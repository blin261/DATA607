...
ggplot(data = claim_cost, aes(x = Claim_Type, y = percentage, fill = State)) + geom_bar(stat = "identity", position = "dodge")
...
TX_meds <- subset(total_cost, total_cost$State == "TX")
head(TX_meds, 10)
ggplot(head(TX_meds, 10), aes(x = reorder(Product.Name, -product_cost), y = product_cost)) + geom_bar(stat = "identity") + ggtitle("TX Highest Cost Medications")+ xlab("Product Name") + ylab("Product Total Cost")
...
```{r}
raw_data2 <- read.table("Medicare_Hospital_Spending_Per_Patient_-_Hospital.csv", sep = ",", stringsAsFactors = FALSE, header = TRUE)

json_url <- "https://raw.githubusercontent.com/blin261/DATA607/master/rows.json"
json_data <- fromJSON(json_url)
patient_spending <- data.frame(json_data, check.rows= FALSE, check.names = FALSE)
