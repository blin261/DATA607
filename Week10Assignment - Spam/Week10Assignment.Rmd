...
tdm <- removeSparseTerms(tdm, 1-(100/length(total_corpus)))
tdm
...
```{r}
set.seed(88)
training_index <- sample(1: length(org_labels), size = round(0.75 * length(org_labels)), replace = FALSE)
training_index
...
#traceback()
container <- create_container(dtm, labels = org_labels, trainSize = training_index, testSize = test_index, virgin = FALSE)
slotNames(container)

#container <- create_container(dtm, labels = org_labels, trainSize = a[1:2924], testSize = a[2925:3898], virgin = FALSE)
# length(dtm$data) == length(dtm$label)
```
...
I constructed a data frame that contains the correct labels, and the labels that are predicted by three models. Then, I used the prop.table function to print out the proportion of those actuals label match the predicted labels and those do not.  According to the table, SVM and Tree model generate higher accuracy compare to Maxent models when used to predict if email is a spam or ham. 
```{r}
